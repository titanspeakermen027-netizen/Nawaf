...
def add_game_channel(guild_id: int, channel_id: int) -> tuple[bool, str]:
    ...
    if len(channels) >= MAX_GAME_CHANNELS:
        return False, f"❌ وصلتي للحد الأقصى: **{MAX_GAME_CHANNELS} رومات** للفعاليات."
    with connect() as con:
        con.execute(
...
class GameChannels(commands.GroupCog, group_name="game-channels"):
    """Administration controls for the channels where events may run."""
    ...
    async def cog_check(self, interaction: discord.Interaction) -> bool:
        ...
        if not can_control_bot(interaction.user):
            await interaction.response.send_message("❌ غير الإدارة أو رتب التحكم في البوت تقدر تبدل رومات الفعاليات.", ephemeral=True)
            return False
        return True

    @app_commands.command(name="add", description="أضف روم مسموح للفعاليات")
    @app_commands.describe(channel="الروم اللي بغيتي تسمح فيه بالفعاليات")
    ...
    @app_commands.command(name="remove", description="حيد روم من رومات الفعاليات")
    ...
    async def remove(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        if remove_game_channel(interaction.guild.id, channel.id):
            await interaction.response.send_message(f"✅ تحيد {channel.mention} من رومات الفعاليات.", ephemeral=True)
        else:
            await interaction.response.send_message("⚠️ هاد الروم ما كانش مضاف.", ephemeral=True)

    @app_commands.command(name="list", description="شوف رومات الفعاليات المسموح بها")
    async def list_channels(self, interaction: discord.Interaction):
        assert interaction.guild is not None
        ids = get_allowed_game_channels(interaction.guild.id)
        if not ids:
            await interaction.response.send_message(
                "ℹ️ مازال ما تحددو حتى روم. الفعاليات حالياً مسموحة فالرومات بشكل عادي.", ephemeral=True
            )
            return
        lines = []
        ...
        await interaction.response.send_message(
            f"🎮 **رومات الفعاليات** — {len(ids)}/{MAX_GAME_CHANNELS}\n" + "\n".join(lines), ephemeral=True
        )
