...
class ApplyFirstModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction):
        answers = {
            str(item.position): item.value.strip()
        ...
        await interaction.response.send_message(
            "✅ تم حفظ الأسئلة الأولى. اضغط **متابعة** لإكمال باقي الأسئلة.",
            view=ContinueApplicationView(self.cog, self.type_id),
            ephemeral=True,
        )
    ...
    def __init__(self, cog, rows):
        options = [
            discord.SelectOption(
                label=r["name"][:100],
                description=(r["description"] or "اضغط للبدء")[:100],
                emoji="📝",
                value=str(r["id"]),
            )
            for r in rows[:25]
        ...
        super().__init__(
            ...
            options=options,
            custom_id="nawaf:application:type",
        )
        self.cog = cog
...
class QuestionEditorModal(discord.ui.Modal):
    def __init__(self, cog, type_id, page, existing):
        ...
        for index in range(5):
            ...
            field = discord.ui.TextInput(
                label=f"السؤال {position}",
                required=False,
                max_length=200,
                default=(q["question"] if q else ""),
            )
            self.add_item(field)
    ...
    async def on_submit(self, interaction):
        ...
        with connect() as con:
            for index, text in enumerate(values):
                ...
                if text:
                    con.execute(
                        "INSERT INTO application_questions(type_id,position,question,required) VALUES(?,?,?,1) ON CONFLICT(type_id,position) DO UPDATE SET question=excluded.question",
                        (self.type_id, position, text),
                    )
                else:
                    con.execute(
                        "DELETE FROM application_questions WHERE type_id=? AND position=?",
                        (self.type_id, position),
                    )
        await interaction.response.send_message(
            f"✅ تم حفظ أسئلة الصفحة {self.page}.",
            ephemeral=True,
        )
...
class Applications(commands.Cog):
    ...
    async def start_application(self, interaction, type_id):
        ...
        if not typ or typ["guild_id"] != interaction.guild.id:
            return await interaction.response.send_message(
                "❌ نوع التقديم غير موجود.", ephemeral=True
            )
        questions = get_questions(type_id)
        if not questions:
            return await interaction.response.send_message(
                "❌ الإدارة لم تضبط الأسئلة بعد.", ephemeral=True
            )
    ...
    async def finish_application(self, interaction, type_id, answers):
        ...
        with connect() as con:
            cur = con.execute(
                "INSERT INTO applications(guild_id,user_id,type_id,answers,status) VALUES(?,?,?,?, 'pending')",
                (
        ...
        review = (
            interaction.guild.get_channel(typ["review_channel_id"])
            if typ["review_channel_id"]
            else None
        )
        if review:
            description = (
            ...
            e = discord.Embed(
                title=f"📝 {typ['name']} • #{app_id}",
                description=description,
                color=color(typ["color"]),
            )
            for q in get_questions(type_id):
                e.add_field(
                    name=q["question"],
                    value=answers.get(str(q["position"]), "—")[:1024],
                    inline=False,
                )
            try:
                await review.send(embed=e, view=ReviewView(self, app_id))
            except discord.HTTPException:
                pass
        await interaction.response.send_message(
            "✅ تم إرسال التقديم للإدارة.", ephemeral=True
        )
    ...
    async def process_decision(self, interaction, application_id, status, reason):
        ...
        with connect() as con:
            ...
            if not row or row["status"] != "pending":
                return await interaction.response.send_message(
                    "❌ هذا التقديم غير متاح للمراجعة.", ephemeral=True
                )
            con.execute(
        ...
        typ = get_type(row["type_id"])
        applicant = interaction.guild.get_member(row["user_id"])
        result_channel = (
            interaction.guild.get_channel(typ["result_channel_id"])
            if typ and typ["result_channel_id"]
            else None
        )
        ...
        applicant_mention = applicant.mention if applicant else f"<@{row['user_id']}>"
        description = f"{applicant_mention}\n\n{result_text}"
        e = discord.Embed(
        ...
        if result_channel:
            try:
                await result_channel.send(embed=e)
                sent = True
            except discord.HTTPException:
                pass
        if applicant:
            try:
                await applicant.send(embed=e)
            except discord.HTTPException:
                pass

        await interaction.response.edit_message(embed=e, view=None)
        await interaction.followup.send(
            "✅ تم حفظ القرار وإرسال النتيجة."
            if sent
            else "⚠️ تم حفظ القرار، لكن لم يتم تحديد/الوصول إلى روم النتائج.",
            ephemeral=True,
        )
    ...
    async def application(self, interaction):
        ...
        if not rows:
            return await interaction.response.send_message(
                "❌ لا توجد أنواع تقديم مفعلة.", ephemeral=True
            )
        await interaction.response.send_message(
            embed=discord.Embed(
                title="📝 التقديم",
                description="اختر نوع التقديم.",
                color=0x5865F2,
            ),
            view=ApplicationPanel(self, rows[:25]),
        )
    ...
    async def create(
        ...
        with connect() as con:
            cur = con.execute(
                "INSERT INTO application_types(guild_id,name,title,description) VALUES(?,?,?,?)",
                (
                    interaction.guild.id,
                    name,
                    f"تقديم {name}",
            ...
            for p in range(1, questions + 1):
                con.execute(
                    "INSERT INTO application_questions(type_id,position,question,required) VALUES(?,?,?,1)",
                    (type_id, p, defaults[p - 1] if p <= 5 else f"السؤال {p}"),
                )
        await interaction.response.send_message(
            f"✅ تم إنشاء **{name}** بمعرف `{type_id}`. تقدر تعدل الأسئلة من `/application-questions`.",
            ephemeral=True,
        )
    ...
    async def questions(self, interaction, type_id: int):
        ...
        if not typ or typ["guild_id"] != interaction.guild.id:
            return await interaction.response.send_message(
                "❌ معرف التقديم غير صحيح.", ephemeral=True
            )
        qs = get_questions(type_id)
        description = (
            f"عدد الأسئلة: **{len(qs)}/{MAX_QUESTIONS}**\n"
            "اختار الصفحة التي تريد تعديلها."
        )
        await interaction.response.send_message(
            embed=discord.Embed(
                title=f"📝 أسئلة {typ['name']}",
                description=description,
                color=0x5865F2,
            ),
            view=QuestionsPageView(self, type_id),
            ephemeral=True,
        )
    ...
    async def review_channel(self, interaction, channel: discord.TextChannel, type_id: int):
        ...
        with connect() as con:
            con.execute(
                "UPDATE application_types SET review_channel_id=? WHERE id=? AND guild_id=?",
                (channel.id, type_id, interaction.guild.id),
            )
    ...
    async def result_channel(self, interaction, channel: discord.TextChannel, type_id: int):
        ...
        with connect() as con:
            con.execute(
                "UPDATE application_types SET result_channel_id=? WHERE id=? AND guild_id=?",
                (channel.id, type_id, interaction.guild.id),
            )
    ...
    async def role(self, interaction, role: discord.Role, type_id: int):
        ...
        with connect() as con:
            con.execute(
                "UPDATE application_types SET accepted_role_id=? WHERE id=? AND guild_id=?",
                (role.id, type_id, interaction.guild.id),
            )
    ...
    async def panel(self, interaction, channel: discord.TextChannel):
        ...
        rows = get_types(interaction.guild.id)
        if not rows:
            return await interaction.response.send_message("❌ أنشئ نوع تقديم أولاً.", ephemeral=True)
        await channel.send(
            embed=discord.Embed(
                title="📝 التقديم",
                description="اختر نوع التقديم.",
    ...
    async def send_result(self, interaction, application_id: int):
        ...
        if not row or row["status"] == "pending":
            return await interaction.response.send_message(
                "❌ التقديم غير موجود أو لم تتم مراجعته بعد.", ephemeral=True
            )
        typ = get_type(row["type_id"])
        ch = (
            interaction.guild.get_channel(typ["result_channel_id"])
        ...
        if not ch:
            return await interaction.response.send_message(
                "❌ لم يتم تحديد روم النتائج.", ephemeral=True
            )
        ok = "✅ مقبول" if row["status"] == "accepted" else "❌ مرفوض"
        description = (
            f"المتقدم: <@{row['user_id']}>\n"
            f"النتيجة: **{ok}**\n"
            f"المراجع: <@{row['reviewer_id']}>"
        )
        if row["review_reason"]:
            description += f"\nالسبب: {row['review_reason']}"
        e = discord.Embed(
            title=f"📢 نتيجة التقديم #{application_id}",
            description=description,
            color=(
        ...
        await interaction.response.send_message("✅ تم إرسال النتيجة.", ephemeral=True)
    ...
    async def listing(self, interaction):
        ...
        rows = get_types(interaction.guild.id)
        text = "\n".join(
            f"• `{r['id']}` — {r['name']} ({len(get_questions(r['id']))}/10 أسئلة)"
            for r in rows
        ) or "لا توجد أنواع."
